...
#pushbullet token : ZJbP4LffOOXSCgAf7vyACjBd2wR7dZbt
apiKey = "ZJbP4LffOOXSCgAf7vyACjBd2wR7dZbt"
p = PushBullet(apiKey)
...
def connectI2c():
    try:
        ...
        p.pushNote(devices[0]["iden"], 'i2c通信に失敗', 'i2c通信に失敗しました。温度計モジュールと配線を確認してください。')
        bool = 0
        return bool
...
def check_notifications():
    if(sensorTemp >= minTemp):
            ...
            parser.add_argument('-s', '--subject', dest='subject', type=str,
                                default='Rei start just now', help='Title') #警告メールのタイトル
            parser.add_argument('-b', '--body', dest='body', type=str,
                                default='Rei started correctly. Thank you', help='Body of the mail.')
                                #警告メールのメール内容
            args = parser.parse_args()
            p.pushNote(devices[0]["iden"], 'Rei start just now', 'Rei started correctly. Thank you')
...
def send_alert_via_gmail(from_addr, to_addr, passwd, msg):
    ...
    try:
        ...
        p.pushNote(devices[0]["iden"], 'SMTP_SSLでの送信に失敗', 'SMTP_SSLでの送信に失敗しました')
    else:
        print "send via TLS..."
    try:
        ...
        p.pushNote(devices[0]["iden"], 'smtplib.SMTPでの送信に失敗', 'smtplib.SMTPでの送信に失敗しました')
...
def working(setTime):
    if(setTime == '0800' or setTime == '1200' or setTime == '1600' or setTime == '2000'  ):
            ...
            parser.add_argument('-t', '--to', dest='to_addr', type=str,
                                ...
                                help='To address') #警告メールの送信元のメールアドレス
            parser.add_argument('-s', '--subject', dest='subject', type=str,
                                default='「れいちゃん」監視システム', help='Title') #警告メールのタイトル
            parser.add_argument('-b', '--body', dest='body', type=str,
                                default='お疲れ様です。\n現在の温度は'+str(sensorTemp)+'度です。\n私はちゃんとお仕事してます、ご安心ください', help='Body of the mail.')
                                #警告メールのメール内容
            args = parser.parse_args()
            p.pushNote(devices[0]["iden"], '正常に稼働中', 'お疲れ様です。\n現在の冷凍庫の温度は'+str(sensorTemp)+'度です。\n私はちゃんとお仕事してます、ご安心ください')
            from_addr = "alert.from.rei@gmail.com" #警告メールの送信先（Gmail）
            sender_name=u'れいちゃん' #メールの送信者の名前
            print "from: %s <%s>" % (sender_name, from_addr)
            #passwd = getpass.getpass()
...
def alert_temprature():
    if(sensorTemp >= minTemp):
            ...
            parser.add_argument('-t', '--to', dest='to_addr', type=str,
                                ...
                                help='To address') #警告メールの送信元のメールアドレス
            parser.add_argument('-s', '--subject', dest='subject', type=str,
                                default='！！警告！！「れいちゃん」監視システム', help='Title') #警告メールのタイトル
            parser.add_argument('-b', '--body', dest='body', type=str,
                                default='大変です。\n温度が'+str(sensorTemp)+'度になったようです。\n正常に動作しているか、直ちにご確認ください。', help='Body of the mail.')
                                #警告メールのメール内容
            args = parser.parse_args()
            p.pushNote(devices[0]["iden"], '異常を感知', '大変です。\n冷凍庫の温度が'+str(sensorTemp)+'度になったようです。\n冷凍庫が正常に動作しているか、直ちにご確認ください。')
            from_addr = "alert.from.rei@gmail.com" #警告メールの送信先（Gmail）
            sender_name=u'れいちゃん' #メールの送信者の名前
            print "from: %s <%s>" % (sender_name, from_addr)
            #passwd = getpass.getpass()
...
def alert_i2c():
    ...
    parser.add_argument('-t', '--to', dest='to_addr', type=str,
                        ...
                        help='To address') #警告メールの送信元のメールアドレス
    parser.add_argument('-s', '--subject', dest='subject', type=str,
                        default='！！モジュールエラー！！「れいちゃん」監視システム', help='Title') #警告メールのタイトル
    parser.add_argument('-b', '--body', dest='body', type=str,
                        default='申し訳ございません。\ni2c通信に失敗しました。\n株式会社合栄企画に連絡して、温度計モジュールと配線の修理を依頼をしてください。', help='Body of the mail.')
                        #警告メールのメール内容
    args = parser.parse_args()
    p.pushNote(devices[0]["iden"], 'モジュールエラー', '申し訳ございません。\ni2c通信に失敗しました。\n株式会社合栄企画に連絡して、温度計モジュールと配線の修理を依頼をしてください。')
    from_addr = "alert.from.rei@gmail.com" #警告メールの送信先（Gmail）
    sender_name=u'れいちゃん' #メールの送信者の名前
    print "from: %s <%s>" % (sender_name, from_addr)
    #passwd = getpass.getpass()
...
#ここから下は音声通知の設定
def startup(minTemp, sensorTemp):
    message='おはようございます。設定温度は'+str(minTemp)+'度、現在温度は'+str(sensorTemp)+'度です。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
...
def sound_working(sensorTemp):
    message='お疲れ様です。現在の温度は'+str(sensorTemp)+'度です。正常に動作しています。私はちゃんとお仕事してます、ご安心ください'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')

def sound_reworking(sensorTemp):
    message='お疲れ様です。現在の温度は'+str(sensorTemp)+'度です。再起動しました。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')
...
def sound_temp_alert(sensorTemp):
    message='大変です！温度が'+str(sensorTemp)+'度になったようです。確認してください。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')
...
def sound_email_alert():
    message='大変です！メール送信ができなくなっています。インターネット接続とメールサーバを確認してください。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')
...
def sound_internet_alert():
    message='大変です！インターネットに接続できません。インターネット接続を確認してください。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')
...
def ifMinus(sensorTemp):
    if sensorTemp.count('-') == 1:
        minusTemp = sensorTemp.replace('-', 'マイナス')
        return minusTemp 
    
if __name__ == '__main__':
    minTemp = 0 #メールを送る最低温度の設定
    temp = connectI2c()
    sensorTemp = str(round((temp / 16.0),2)))
    sensorTemp = ifMinus(sensorTemp)
    startup(minTemp, sensorTemp)
    check_notifications()
    while True:
        temp = connectI2c()
        if temp:
