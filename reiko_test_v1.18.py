...
def connectI2c():
    try:
        ...
        #    temp -= 8192
        #return temp
        temp = random.randint(-50.00, -40.00) 
        return temp
    except:
...
def send_via_gmail(from_addr, to_addr, passwd, msg):
    ...
    try:
        ...
        sound_email_alert()
        print 'SMTPでの送信に失敗しました。'
        p.pushNote(devices[0]["iden"], 'SMTPでの送信に失敗', 'SMTPでの送信に失敗しました')
...
def working(setTime):
    if(setTime == '0800' or setTime == '1200' or setTime == '1600' or setTime == '2000'  ):
            ...
            p.pushNote(devices[0]["iden"], '正常に稼働中', 'ご主人様、お疲れ様です。\n現在の冷凍庫の温度は'+str(sensorTemp)+'度です。\n私はちゃんとお仕事してます、ご安心ください')
...
def alert_temprature():
    if(sensorTemp >= minTemp):
            ...
            parser.add_argument('-b', '--body', dest='body', type=str,
                                ...
                                #警告メールのメール内容
            args = parser.parse_args()
            p.pushNote(devices[0]["iden"], '異常を感知', 'ご主人様大変ですっ。\n冷凍庫の温度が'+str(sensorTemp)+'度になったようです。\n冷凍庫が正常に動作しているか、直ちにご確認ください。')
            from_addr = "alert.from.rei@gmail.com" #警告メールの送信先（Gmail）
            sender_name=u'れいちゃん' #メールの送信者の名前
            print "from: %s <%s>" % (sender_name, from_addr)
            #passwd = getpass.getpass()
...
def alert_i2c():
    ...
    parser.add_argument('-b', '--body', dest='body', type=str,
                        ...
                        #警告メールのメール内容
    args = parser.parse_args()
    p.pushNote(devices[0]["iden"], 'モジュールエラー', 'ご主人様申し訳ございません。\ni2c通信に失敗しました。\n株式会社合栄企画に連絡して、温度計モジュールと配線の修理を依頼をしてください。')
    from_addr = "alert.from.rei@gmail.com" #警告メールの送信先（Gmail）
    sender_name=u'れいちゃん' #メールの送信者の名前
    print "from: %s <%s>" % (sender_name, from_addr)
    #passwd = getpass.getpass()
...
#ここから下は音声通知の設定
def startup():
    p.pushNote(devices[0]["iden"], '起動しました', 'れいこは正常に起動しました')
    message='ご主人様、お疲れ様です。起動しました。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
