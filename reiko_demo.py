...
#動作しているということの定時連絡
def working(setTime):
    if(setTime == '0800' or setTime == '1200' or setTime == '1515' or setTime == '2000'  ):
            print("I am working")
...
def startup(minTemp, sensorTemp):
    message='ご主人様、おはようございます。設定温度は'+str(minTemp)+'度、現在温度は'+str(sensorTemp)+'度です。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
...
def sound_working(sensorTemp):
    message='ご主人様、お疲れ様です。現在の温度は'+str(sensorTemp)+'度です。正常に動作しています。私はちゃんとお仕事してます、ご安心ください'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')
...
def sound_reworking(sensorTemp):
    message='ご主人様、お疲れ様です。現在の温度は'+str(sensorTemp)+'度です。再起動しました。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')
...
def sound_temp_alert(sensorTemp):
    message='ご主人様大変です！温度が'+str(sensorTemp)+'度になったようです。確認してください。'
    f = open('alert.txt', 'w')
    f.write(message)
    f.close()
    os.system('python rei_voice.py alert.txt')
...
def convertToMainasu(sensorTemp):
    if sensorTemp.count('-') == 1:
        minusTemp = sensorTemp.replace('-', 'マイナス')
        return minusTemp

def convertToMinus(sensorTemp):
    if sensorTemp.count('マイナス') == 1:
        minusTemp = sensorTemp.replace('マイナス', '-')
        return minusTemp 
...
if __name__ == '__main__':
    ...
    sensorTemp = convertToMainasu(sensorTemp)
